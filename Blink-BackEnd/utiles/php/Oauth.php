...
class Oauth {
    ...
    protected $alt_code;
    protected $alt_state;
    protected $alt_oauth_token;
    protected $alt_oauth_verifier;
    ...
    public function makeRequest($url, $method = 'GET', Array $parameters = array(), $returnType = 'json', $includeCallback = false, $includeVerifier = false) {
        ...
        if (isset($this->_request_token_url) && strlen($this->_request_token_url) > 0) {
            ...
            $oauth_verifier = isset($_GET['oauth_verifier']) ? isset($_GET['oauth_verifier']) : $this->alt_oauth_verifier;
            if ($includeVerifier && isset($oauth_verifier)) {
                $headers['oauth_verifier'] = $oauth_verifier;
            }
    ...
    public function validateAccessToken() {
        ...
        if (!isset($this->_access_token) || strlen($this->_access_token) == 0) {
            ...
            $this->getAltCodes();
            $oauth_verifier = isset($_GET['oauth_verifier']) ? isset($_GET['oauth_verifier']) : $this->alt_oauth_verifier;
            $oauth_token = isset($_GET['oauth_token']) ? isset($_GET['oauth_token']) : $this->alt_oauth_token;
            ...
            if (isset($this->_request_token_url) && strlen($this->_request_token_url) > 0) {
                ...
                // request access token
                    ...
                    if ($oauth_token != $_SESSION[$this->_prefix]['token']) {
            ...
            // handle oauth 2.0 flow
                ...
                $this->getAltCodes();
                $code = isset($_GET['code']) ? $_GET['code'] : $this->alt_code;
                $state = isset($_GET['state']) ? $_GET['state'] : $this->alt_state;
                // authorize app
                if (!isset($code) && !isset($state)) {
                    //if ((!isset($_GET['state']) && !isset($_GET['code'])) && (!isset($this->alt_state) && !isset($this->alt_code))) {
                    $this->authorize($this->_scope);
                    return false;
                }
                // request access token
                else {
                    if (($state != $_SESSION[$this->_prefix]['state'])) {
                        // if (($dato != $_SESSION[$this->_prefix]['state']) && ($this->alt_state != $_SESSION[$this->_prefix]['state'])) {
    ...
    private function getAltCodes() {
        $str1 = $_SERVER['REQUEST_URI'];
        $arr1 = explode('?', $str1);
        if ($arr1 && isset($arr1[1])) {
            $arr2 = explode('&', $arr1[1]);
            if (substr($arr2[0],0, 4) == "code") {
                $this->alt_code = substr($arr2[0], 5);
            }            
            if (substr($arr2[1],0, 4) == "code") {
                $this->alt_code = substr($arr2[1], 5);
            }
            
            if (substr($arr2[1],0, 5) == "state") {
                $this->alt_state = substr($arr2[1], 6);
            }
            if (substr($arr2[0],0, 5) == "state") {
                $this->alt_state = substr($arr2[0], 6);
            }

            
            if (substr($arr2[0],0, 11) == "oauth_token") {
                $this->alt_oauth_token = substr($arr2[0], 12);
            }
            if (substr($arr2[1],0, 11) == "oauth_token") {
                $this->alt_oauth_token = substr($arr2[1], 12);
            }
            
            if (substr($arr2[0],0, 14) == "oauth_verifier") {
                $this->alt_oauth_verifier = substr($arr2[0], 15);
            }
            if (substr($arr2[1],0, 14) == "oauth_verifier") {
                $this->alt_oauth_verifier = substr($arr2[1], 15);
            }
